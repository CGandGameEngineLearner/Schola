...
def get_files(folder):
    return [ file_name for file_name in os.listdir(folder) if isfile(join(folder,file_name))]

def get_proto_files(folder):
    return [ file_name for file_name in get_files(folder) if file_name.endswith(".proto")]
    ...
    for file in files:
        with open(join(folder,file), "r+") as f:
            ...
            for i,line in enumerate(file_contents):
                ...
                if result:
                    file_contents[i] = f"import schola.generated.{result.group(1)} as {result.group(2)}\n"
            f.seek(0)
...
def disable_warnings(folder, file_paths, warnings):
    for file_path in file_paths:
        with open(join(folder,file_path), "r+") as f:
            ...
            try:
                headers_start_index = next((line_num for line_num,line in enumerate(file_contents) if line.startswith("#include ")))
            except StopIteration:
                print(file_path)
                headers_start_index = 3
            for warning in warnings:
                file_contents.insert(headers_start_index,f"#pragma warning (disable : {warning})\n")
            f.seek(0)
            f.writelines(file_contents)
            f.truncate()
